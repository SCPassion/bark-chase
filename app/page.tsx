        ...
        <div className="relative group">
          ...
          <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-64 sm:w-72 md:w-80 px-4 py-3 bg-gray-900 border-2 border-chase-accent rounded-lg shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none z-50">
            <p className="text-xs sm:text-sm text-white font-medium text-left mb-2">
              💡 You need{" "}
              <span className="font-bold text-chase-accent">$CHASE</span> token
              in your Fogo wallet to make Chase smile
            </p>
            <p className="text-xs text-white font-medium text-left">
              🔥 Each click burns{" "}
              <span className="font-semibold text-chase-accent">1 $CHASE</span>{" "}
              token, making it deflationary
            </p>
...
export default function Home() {
  return (
    <main className="min-h-screen bg-chase-bg flex flex-col items-center p-4 sm:p-6 md:p-8 pt-16 sm:pt-20 md:pt-24 pb-12 md:pb-16">
      {/* Live on Fogo + stats strip (Fogo-style) */}
      <div className="w-full max-w-5xl mx-auto mb-6 md:mb-8">
        <p className="inline-block px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider text-chase-accent border-2 border-chase-accent/50 bg-chase-accent/10 mb-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          {STATS.map(({ value, label }) => (
            <div
              key={label}
              className="rounded-xl border-2 border-chase-accent/30 bg-gray-900/50 px-4 py-3 text-center"
            >
              <p className="text-lg md:text-xl font-black text-chase-accent tabular-nums">
                {value}
              </p>
              <p className="text-xs font-medium text-chase-muted">{label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-8 text-center">
        <Link
          href="/intro"
          className="text-sm font-medium text-chase-muted hover:text-chase-accent transition-colors"
        >
          What is Chase Dog? Learn more →
        </Link>
      </div>
    </main>
  );
